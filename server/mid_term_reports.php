...
*/ 
    ...
    public function save(){
        $db = new DbHandlers();
        $sql = "INSERT INTO mid_term_reports(";
    ...
    public function update($pvcol, $pval){
        $db = new DbHandlers();
        $sql = "UPDATE mid_term_reports SET ";
        if (isset($this->id) && $this->id!=="" ) {
             $sql.= " id = '{$this->id}'";    
        }
        if (isset($this->student_id) && $this->student_id!=="" ) {
    ...
    public function view($critcol=null, $critval=null) {
        $db = new DbHandlers();
        if(is_null($critcol)){
            $sql = "SELECT * from mid_term_reports order by id DESC";
        } else {
        $sql = "SELECT * from mid_term_reports WHERE $critcol ='{$critval}'";
        }
    ...
    public  function delete($critcol, $critval){
        $db = new DbHandlers();
        $sql = "DELETE FROM mid_term_reports WHERE $critcol ='{$critval}'";
