...
class Primary_first_term_report{
...
*/ 
    ...
    public function save(){
        $db = new DbHandlers();
        $sql = "INSERT INTO primary_first_term_reports(";
        ...
        if (isset($this->second_test) && $this->second_test!=="" ) {
            $sql.= ',second_test';    
        }
    ...
    public function update($pvcol, $pval){
        $db = new DbHandlers();
        $sql = "UPDATE primary_first_term_reports SET ";
        if (isset($this->id) && $this->id!=="" ) {
             $sql.= " id = '{$this->id}'";    
        }
        if (isset($this->student_id) && $this->student_id!=="" ) {
        ...
        if (isset($this->second_test) && $this->second_test!=="" ) {
            $sql.= ", second_test = '{$this->second_test}'";    
        }
        if (isset($this->total_test) && $this->total_test!=="" ) {
    ...
    public function view($critcol=null, $critval=null) {
        $db = new DbHandlers();
        if(is_null($critcol)){
            $sql = "SELECT * from primary_first_term_reports order by id DESC";
        } else {
        $sql = "SELECT * from primary_first_term_reports WHERE $critcol ='{$critval}'";
        }
    ...
    public  function delete($critcol, $critval){
        $db = new DbHandlers();
        $sql = "DELETE FROM primary_first_term_reports WHERE $critcol ='{$critval}'";
